// Squeezes the four stock doors and spaces them CSS_DOOR_PITCH apart from
// door 0's stock position so six fit, moving each door's pieces by joint
...
// Door 0's stock HMN and team button bounds relative to its background.
...
void css_rescale_doors() {
    ...
    // The stock name boxes sit at fixed positions, so they cannot follow the
